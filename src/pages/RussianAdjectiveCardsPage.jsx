...
import { RUSSIAN_ADJECTIVE_CARDS } from '../utils/russianAdjectiveCards';
...
function buildOptions(card, allCards) {
  ...
  const pool = shuffleArray(
    allCards.filter(c => c.id !== card.id).map(c => c.ru)
  ).slice(0, 3);
...
const BrowseMode = ({ language, onHome }) => {
  ...
  const handleSpeak = (e, card) => {
    e.stopPropagation();
    speak(card.ru, 'ru-RU', `ru-adj-${card.id}`);
  };
  ...
  return (
    <>
      <p className="acp-subtitle">
        {language === 'es'
          ? 'Haz clic en una tarjeta para ver la traducción en ruso'
          : 'Click a card to reveal the Russian translation'}
      </p>
      <div className="acp-grid">
        {RUSSIAN_ADJECTIVE_CARDS.map(card => (
          <div
            key={card.id}
          ...
          >
            <div className="acp-card-inner">
              ...
              <div className="acp-card-back">
                <span className="acp-card-target">{card.ru}</span>
                <span className="acp-card-translit">{card.translit}</span>
                {supported && (
                  <button
                    className="acp-tts-btn"
                    onClick={(e) => handleSpeak(e, card)}
                    aria-label={`Pronounce ${card.ru}`}
                  >
                    {speakingId === `ru-adj-${card.id}` ? '🔊' : '🔈'}
                  </button>
                )}
...
const QuizMode = ({ language, onHome }) => {
  ...
  const [cards] = useState(() => shuffleArray(RUSSIAN_ADJECTIVE_CARDS));
  ...
  const { options, correctIndex } = useMemo(
    () => buildOptions(card, RUSSIAN_ADJECTIVE_CARDS),
    [card]
  );
  ...
  const feedbackText = selected !== null
    ? selected === correctIndex
      ...
      : `${language === 'es' ? 'Incorrecto — la respuesta es' : 'Incorrect — the answer is'} ${card.ru}`
    : '';
    ...
    <>
      ...
      <div className="acp-quiz-card">
        ...
        <p className="acp-quiz-prompt">
          {language === 'es' ? '¿Cómo se dice en ruso?' : 'How do you say in Russian?'}
        </p>
      ...
      {selected !== null && (
        <div style={{ display: 'flex', justifyContent: 'center', gap: '0.75rem', alignItems: 'center' }}>
          {supported && (
            <button
              className="acp-tts-btn"
              onClick={() => speak(card.ru, 'ru-RU', `ru-quiz-${card.id}`)}
              aria-label={`Pronounce ${card.ru}`}
            >
              {speakingId === `ru-quiz-${card.id}` ? '🔊' : '🔈'}
            </button>
          )}
...
const RussianAdjectiveCardsPage = ({ onHome }) => {
...
export default RussianAdjectiveCardsPage;
